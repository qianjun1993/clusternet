...
func calculateReplicas(
	...
	nodeScoreList, calculate := fwk.RunCalculatePlugins(ctx, requirements, nodes, availableList)
	...
	if klog.V(10).Enabled() {
		for i := range availableList {
			klog.V(10).InfoS("calculate node's max available replicas", "nodes", nodeScoreList[i].Name, "max available replicas", availableList[i].MaxAvailableReplicas)
		}
	}
