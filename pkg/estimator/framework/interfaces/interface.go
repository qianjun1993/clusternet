...
// after requirements cannot be scheduled.
type PostFilterPlugin interface {
	Plugin

	// PostFilter is called by the scheduling framework.
	// A PostFilter plugin should return one of the following statuses:
	// - Unschedulable: the plugin gets executed successfully but the requirements cannot be made schedulable.
	// - Success: the plugin gets executed successfully and the requirements can be made schedulable.
...
// PreCalculatePlugin is an interface for "PreCalculate" plugin. PreCalculate is an
// informational extension point. These are meant to prepare the state of
// the calculate.
type PreCalculatePlugin interface {
	Plugin

	// PreCalculate is called by the scheduling framework after a list of managed nodes
	// passed the filtering phase. All pre-calculate plugins must return success or
	// the requirements will be rejected.
	PreCalculate(ctx context.Context, requirements *appsapi.ReplicaRequirements, nodesInfo []*NodeInfo) *Status
}

// CalculatePlugin is an interface that must be implemented by "Calculate" plugins to
// calculate acceptable replicas for each filtered node.
type CalculatePlugin interface {
	Plugin

	// Calculate is called on each filtered node. It will calculate acceptable replicas
	// for each filtered node.
	Calculate(ctx context.Context, requirements *appsapi.ReplicaRequirements, nodeInfo *NodeInfo) (int32, *Status)
}
...
type PreAggregatePlugin interface {
	...
	// PreAggregate is called by the scheduling framework after a list of managed Nodes
...
type Framework interface {
	...
	// cluster state to make the requirements potentially schedulable in a future scheduling cycle.
	...
	// RunPreCalculatePlugins runs the set of configured PreCalculate plugins. It returns
	// *Status and its code is set to non-success if any of the plugins returns
	// anything but Success. If a non-success status is returned, then the scheduling
	// cycle is aborted.
	RunPreCalculatePlugins(ctx context.Context, requirements *appsapi.ReplicaRequirements, nodesInfo []*NodeInfo) *Status

	// RunCalculatePlugins runs the set of configured Calculate plugins. It returns one number that
	// stores for each Calculate plugin. It also returns *Status, which is set to non-success
	// if any of the plugins returns a non-success status.
	RunCalculatePlugins(ctx context.Context, requirements *appsapi.ReplicaRequirements, nodesInfo []*NodeInfo, availableList NodeScoreList) (NodeScoreList, *Status)
...
// scheduling the requirements on nodes when certain running pods get evicted.
