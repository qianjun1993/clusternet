...
type frameworkImpl struct {
	...
	preCalculatePlugins []framework.PreCalculatePlugin
	calculatePlugins    []framework.CalculatePlugin
...
func (f *frameworkImpl) getExtensionPoints(plugins *estimatorapis.Plugins) []extensionPoint {
	return []extensionPoint{
		...
		{&plugins.PreCalculate, &f.preCalculatePlugins},
		{&plugins.Calculate, &f.calculatePlugins},
...
// RunPreCalculatePlugins runs the set of configured pre-calculate plugins. If any
// of these plugins returns any status other than "Success", the given requirement is rejected.
func (f *frameworkImpl) RunPreCalculatePlugins(ctx context.Context, requirements *appsapi.ReplicaRequirements, nodesInfo []*framework.NodeInfo) (status *framework.Status) {
	startTime := time.Now()
	defer func() {
		metrics.FrameworkExtensionPointDuration.WithLabelValues(preCalculate, status.Code().String(), f.profileName).Observe(metrics.SinceInSeconds(startTime))
	}()
	for _, pl := range f.preCalculatePlugins {
		status = f.runPreCalculatePlugin(ctx, pl, requirements, nodesInfo)
		if !status.IsSuccess() {
...
func (f *frameworkImpl) runPreCalculatePlugin(ctx context.Context, pl framework.PreCalculatePlugin, requirements *appsapi.ReplicaRequirements, nodesInfo []*framework.NodeInfo) *framework.Status {
	startTime := time.Now()
	status := pl.PreCalculate(ctx, requirements, nodesInfo)
	f.metricsRecorder.observePluginDurationAsync(preCalculate, pl.Name(), status, metrics.SinceInSeconds(startTime))
	return status
}

// RunCalculatePlugins runs the set of configured calculating plugins. It returns a list that
// stores for each Calculate plugin name the corresponding NodeScoreList(s).
// It also returns *Status, which is set to non-success if any of the plugins returns
// a non-success status.
func (f *frameworkImpl) RunCalculatePlugins(ctx context.Context, requirements *appsapi.ReplicaRequirements, nodesInfo []*framework.NodeInfo, availableList framework.NodeScoreList) (res framework.NodeScoreList, status *framework.Status) {
	startTime := time.Now()
	defer func() {
		metrics.FrameworkExtensionPointDuration.WithLabelValues(calculate, status.Code().String(), f.profileName).Observe(metrics.SinceInSeconds(startTime))
	}()
	...
	f.Parallelizer().Until(ctx, len(nodesInfo), func(index int) {
		for i, pl := range f.calculatePlugins {
			replicas, status := f.runCalculatePlugin(ctx, pl, requirements, nodesInfo[index])
			if !status.IsSuccess() {
...
func (f *frameworkImpl) runCalculatePlugin(ctx context.Context, pl framework.CalculatePlugin, requirements *appsapi.ReplicaRequirements, nodeInfo *framework.NodeInfo) (int32, *framework.Status) {
	startTime := time.Now()
	replicas, status := pl.Calculate(ctx, requirements, nodeInfo)
	f.metricsRecorder.observePluginDurationAsync(calculate, pl.Name(), status, metrics.SinceInSeconds(startTime))
...
func (f *frameworkImpl) RunAggregatePlugins(ctx context.Context, requirements *appsapi.ReplicaRequirements, scores framework.NodeScoreList) (result framework.AcceptableReplicas, status *framework.Status) {
	...
	for _, ap := range f.aggregatePlugins {
		result, status = f.runAggregatePlugin(ctx, ap, requirements, scores)
...
// HasCalculatePlugins returns true if at least one calculate plugin is defined.
func (f *frameworkImpl) HasCalculatePlugins() bool {
	return len(f.calculatePlugins) > 0
}
...
// HasAggregatePlugins returns true if at least one calculate plugin is defined.
