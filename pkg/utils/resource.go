...
func NewNodeInfo(node *corev1.Node) *NodeInfo {
	allocatableResource := NewResource(node.Status.Allocatable)
	return &NodeInfo{
		Node:                node,
...
func (ni *NodeInfo) AddPodRequest(pods []*corev1.Pod) error {
	occupiedResource := EmptyResource()
	...
	// subtract
	if ni.IdleResource.Less(occupiedResource) {
...
func (ni *NodeInfo) AddReplicaSetRequest(rl corev1.ResourceList, replica int64) error {
	occupiedResource := NewResource(rl)
	occupiedResource.Multi(replica)
	occupiedResource.AddResourcePods(replica)

	// subtract
	if ni.IdleResource.Less(occupiedResource) {
		return fmt.Errorf("node %s does have enough idle resource to accommodate %d replicas", ni.Node.Name, replica)
	}
	if err := ni.IdleResource.Sub(occupiedResource.ResourceList()); err != nil {
		return err
	}
